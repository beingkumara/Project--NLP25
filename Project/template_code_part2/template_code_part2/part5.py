...
def build_graded_relevant_docs(all_qrels_list):
    # Let us build the graded relevant docs here
    graded_rels = {}
    for i in range(len(all_qrels_list)):
        ...
        elif grade_value > graded_rels[query_id][doc_id]:
            graded_rels[query_id][doc_id] = grade_value
    return graded_rels


def compute_graded_ndcg_for_query(doc_IDs_ordered, graded_rels_for_query, k):
    # Executing logic for computing graded ndcg for query
    dcg_score = 0.0
    idcg_score = 0.0
    top_k_docs = []
    ...
    for ideal_idx in range(ideal_count):
        ...
        if ideal_gain > 0:
            idcg_score = idcg_score + float(ideal_gain) / math.log2(
                position_1indexed + 1
            )
    if idcg_score == 0.0:
        return 0.0
...
def compute_mean_graded_ndcg(doc_IDs_ordered_dict, query_ids, graded_rels, k):
    # Let us compute mean graded ndcg
    if len(query_ids) == 0:
        return 0.0
...
def preprocess_all(query_texts, doc_texts):
    # Basically preprocessing everything here
...
def compute_all_metrics(evaluator, doc_IDs_ordered_dict, query_ids, relevant_docs, k):
    # Executing logic to compute all metrics
    ...
    m_map = evaluator.meanAveragePrecision(
        doc_IDs_ordered_dict, query_ids, relevant_docs, k
    )
    ndcg = evaluator.meanNDCG(doc_IDs_ordered_dict, query_ids, relevant_docs, k)
    mrr = evaluator.meanReciprocalRank(
        doc_IDs_ordered_dict, query_ids, relevant_docs, k
    )
    return {
        "precision": prec,
...
def wilcoxon_test(ap_baseline, ap_method, method_name, alternative="greater"):
    # Let us execute the wilcoxon test logic
    differences = []
    ...
    if len(non_zero) == 0:
        return {
            ...
            "H0": "AP scores of "
            + method_name
            + " and VSM baseline come from the same distribution.",
    ...
    if alternative == "greater":
        h1_str = (
            method_name + " has significantly higher AP scores than the VSM baseline."
        )
    else:
        h1_str = (
            method_name
            + " has significantly different AP scores from the VSM baseline (two-sided)."
        )
    if significant:
        interpretation = (
            str(method_name)
            + " IS significantly different from VSM baseline"
            + " (alternative="
            + alternative
            + ", p="
            + str(round(p_value, 4))
            + ", mean delta MAP="
            + str(round(mean_delta, 4))
            + ", Cohen's d="
            + str(round(cohens_d, 3))
            + " ["
            + effect_label
            + "] effect). Reject H0."
        )
        ...
        interpretation = (
            str(method_name)
            + " is NOT significantly different from VSM baseline"
            + " (alternative="
            + alternative
            + ", p="
            + str(round(p_value, 4))
...
def plot_metrics_vs_k(all_systems_metrics_over_k, k_values, out_path):
    # Executing logic to plot metrics vs k
...
def plot_ablation(ablation_results, out_path):
    ...
    # Let us plot the ablation results
    ...
    axes[0].grid(True)
    axes[1].plot(
        actual_k_vals, ndcgs, "-s", color="darkorange", label="nDCG@10 for LSA"
    )
    if baseline_ndcg is not None:
        axes[1].axhline(
            baseline_ndcg, color="red", linestyle="--", label="VSM Baseline"
        )
...
def plot_final_bar_chart(final_metrics, out_path):
    # Executing logic to plot final bar chart
...
def print_standardized_formulation(system_name, measures_list):
    # Here we print the standardized formulation
    measures_str = ""
...
def per_query_granularity_analysis(
    per_query_ap_dict, query_ids, query_text_dict, n_top=5
):
    # Basically executing the per query granularity analysis
...
def systematic_failure_analysis(
    per_query_ap_dict, query_ids, query_text_dict, relevant_docs
):
    # Executing logic for systematic failure analysis
    baseline_ap = per_query_ap_dict["VSM (Baseline)"]
    failure_report = {}
    system_names = []
    for name in per_query_ap_dict.keys():
        ...
        for q_idx in range(len(query_ids)):
            ...
            if b_ap > 0.001 and m_ap < 0.001:
                ...
                if rel_count <= 2:
                    ...
                    cause_hint = (
                        "sparse coverage in qrels may amplify any small ranking change"
                    )
                elif q_length <= 3:
...
def marginal_difference_analysis(
    final_metrics, per_query_ap_dict, query_ids, query_text_dict
):
    # Finding the max value first
    # Let us run the marginal difference analysis
    marginal_pairs = []
...
def qualitative_case_study(
    all_ranked_dicts, query_ids, all_qrels, query_text_dict, doc_text_dict
):
    # Basically executing the qualitative case study
    case_studies = {}
    target_query_id = 1
    target_doc_id = 29
...
def run_bm25_cross_validation(
    ...
    # First we need to handle this edge case condition
    # Let us run the bm25 cross validation
    ...
    for fold_index in range(5):
        ...
        for k_idx in range(len(k1_options)):
            ...
            for b_idx in range(len(b_options)):
                ...
                train_map = evaluator.meanAveragePrecision(
                    temp_dict, train_q_ids, relevant_docs, k_eval
                )
                combo_key = (current_k1, current_b)
                combo_cumulative_map[combo_key] = (
                    combo_cumulative_map[combo_key] + train_map
                )
        ...
        print(
            "    Fold",
            fold_index + 1,
            ": Best train params found: k1=",
            best_fold_k1,
            "b=",
            best_fold_b,
            ". Now evaluating on held-out test fold...",
        )
        ...
        test_map = evaluator.meanAveragePrecision(
            test_dict, test_q_ids, relevant_docs, k_eval
        )
    ...
    print(
        "\n  Globally best hyperparameters selected by CV: k1=",
        best_overall_k1,
        "b=",
        best_overall_b,
    )
    print(
        "  Overall robust Cross-Validated test MAP@10 estimate:",
        round(overall_avg_test_map, 4),
    )
    print(
        "  These params will now be used for the final evaluation on the full dataset."
    )
    return (best_overall_k1, best_overall_b)


def run_lsa_cross_validation(
    processed_queries,
    processed_docs,
    doc_ids,
    query_ids,
    ...
    # Executing logic for run lsa cross validation
    ...
    for fold_index in range(5):
        ...
        print(
            ...
            best_fold_k,
            "(train MAP =",
            round(best_fold_map, 4),
            "). Evaluating on held-out test fold...",
        )
    ...
    print(
        "  Overall Cross-Validated test MAP@10 estimate:",
        round(overall_avg_test_map, 4),
    )
...
def main():
    ...
    query_ids, query_texts, doc_ids, doc_texts = parse_cranfield_data(
    ...
    all_systems_metrics_over_k = {}
    final_metrics_at_k10 = {}
    per_query_ap_at_k10 = {}
    graded_ndcg_at_k10 = {}
    eval_measures = [
        "Precision@k",
    ...
    for k_idx in range(len(k_range)):
        k = k_range[k_idx]
        vsm_metrics_k[k] = compute_all_metrics(
            evaluator, vsm_dict, query_ids, relevant_docs, k
        )
    ...
    print(
        "  Calculated MAP@10 =",
        round(vsm_metrics_k[EVAL_K]["map"], 4),
        "| nDCG@10 =",
        round(vsm_metrics_k[EVAL_K]["ndcg"], 4),
        "| nDCG@10 (graded) =",
        round(graded_ndcg_at_k10["VSM (Baseline)"], 4),
        "| MRR@10 =",
        round(vsm_metrics_k[EVAL_K]["mrr"], 4),
    )
    print(
        "\n[4a/6] Running METHOD A (BM25, k1="
        + str(cv_best_k1)
        + ", b="
        + str(cv_best_b)
        + ")..."
    )
    print_standardized_formulation(
        "BM25 (k1=" + str(cv_best_k1) + ", b=" + str(cv_best_b) + ")", eval_measures
    )
    t0 = time.time()
    ...
    for k_idx in range(len(k_range)):
        k = k_range[k_idx]
        bm25_metrics_k[k] = compute_all_metrics(
            evaluator, bm25_dict, query_ids, relevant_docs, k
        )
    ...
    print(
        "  Calculated MAP@10 =",
        round(bm25_metrics_k[EVAL_K]["map"], 4),
        "| MRR@10 =",
        round(bm25_metrics_k[EVAL_K]["mrr"], 4),
        "| nDCG@10 (graded) =",
        round(graded_ndcg_at_k10["BM25"], 4),
    )
    print("\n[4b/6] Running METHOD B (LSA) — Ablation plot over n_components...")
    print_standardized_formulation(
        "LSA (Truncated SVD, CV-selected k=" + str(cv_best_lsa_k) + ")", eval_measures
    )
    ablation_results = {}
    ...
    for i in range(len(LSA_K_VALUES)):
        ...
        lsa_m = compute_all_metrics(
            evaluator, lsa_dict, query_ids, relevant_docs, EVAL_K
        )
        print(
            "  MAP@10 =",
            round(lsa_m["map"], 4),
    ...
    for k_idx in range(len(k_range)):
        k = k_range[k_idx]
        lsa_metrics_k[k] = compute_all_metrics(
            evaluator, lsa_best_dict, query_ids, relevant_docs, k
        )
    ...
    print(
        "  MAP@10 =",
        round(lsa_metrics_k[EVAL_K]["map"], 4),
        "| MRR@10 =",
        round(lsa_metrics_k[EVAL_K]["mrr"], 4),
        "| nDCG@10 (graded) =",
        round(graded_ndcg_at_k10[lsa_sys_name], 4),
    )
    ...
    print(
        "  Calculated MAP@10 =",
        round(exp_metrics_k[EVAL_K]["map"], 4),
        "| MRR@10 =",
        round(exp_metrics_k[EVAL_K]["mrr"], 4),
        "| nDCG@10 (graded) =",
        round(graded_ndcg_at_k10["VSM + QE"], 4),
    )
    ...
    print(
        "  Completed Pseudo-Relevance Feedback in",
    ...
    for k_idx in range(len(k_range)):
        k = k_range[k_idx]
        prf_metrics_k[k] = compute_all_metrics(
            evaluator, prf_dict, query_ids, relevant_docs, k
        )
    ...
    print(
        "  Calculated MAP@10 =",
        round(prf_metrics_k[EVAL_K]["map"], 4),
        "| MRR@10 =",
        round(prf_metrics_k[EVAL_K]["mrr"], 4),
        "| nDCG@10 (graded) =",
        round(graded_ndcg_at_k10["BM25 + PRF"], 4),
    )
    ...
    for k_idx in range(len(k_range)):
        k = k_range[k_idx]
        esa_metrics_k[k] = compute_all_metrics(
            evaluator, esa_dict, query_ids, relevant_docs, k
        )
    ...
    print(
        "  Calculated MAP@10 =",
        round(esa_metrics_k[EVAL_K]["map"], 4),
        "| MRR@10 =",
        round(esa_metrics_k[EVAL_K]["mrr"], 4),
        "| nDCG@10 (graded) =",
        round(graded_ndcg_at_k10["ESA"], 4),
    )
    ...
    for k_idx in range(len(k_range)):
        k = k_range[k_idx]
        esac_metrics_k[k] = compute_all_metrics(
            evaluator, esac_dict, query_ids, relevant_docs, k
        )
    ...
    print(
        "  Calculated MAP@10 =",
        round(esac_metrics_k[EVAL_K]["map"], 4),
        "| MRR@10 =",
        round(esac_metrics_k[EVAL_K]["mrr"], 4),
        "| nDCG@10 (graded) =",
        round(graded_ndcg_at_k10["ESA (Cranfield)"], 4),
    )
    print("\n[5/6] Running Wilcoxon Signed-Rank Tests...")
    print("  Formal Hypothesis Framework:")
    print(
        "    H0: The AP scores of method X and the VSM baseline come from the same distribution."
    )
    print(
        "    H1 (one-sided): Method X produces significantly higher AP scores (for BM25, LSA, PRF)."
    )
    print(
        "    H1 (two-sided): Method X produces significantly different AP scores (for QE, ESA)."
    )
    ...
    systems_and_alternatives = [
        ("BM25", "greater"),
        (lsa_sys_name, "greater"),
        ("VSM + QE", "two-sided"),
        ("BM25 + PRF", "greater"),
        ("ESA", "two-sided"),
        ("ESA (Cranfield)", "two-sided"),
    ]
    ...
    if "worst_ranked_relevant" in case_studies:
        ...
        print(
            "  -> Query (ID " + str(wc["query_id"]) + "):",
            wc["query_text"][:80],
            "... (Doc ID " + str(wc["doc_id"]) + ")",
        )
    ...
    for pq_idx in range(len(pq_sys_names)):
        ...
        print(
            ...
            sys_name,
            ": improved on",
            info["queries_improved"],
            "queries, degraded on",
            info["queries_degraded"],
            "queries, unchanged on",
            info["queries_unchanged"],
            "queries.",
        )
    ...
    for fa_idx in range(len(fa_sys_names)):
        ...
        print(
            ...
            sys_name,
            ":",
            fr["new_failures_count"],
            "new failures vs baseline,",
            fr["fixed_failures_count"],
    ...
    plot_metrics_vs_k(
        all_systems_metrics_over_k, k_range, OUT_DIR + "part5_comparison_map_ndcg.png"
    )
